...
phenome_1=np.where(phoneme_id==p_id)
X_phoneme_1=[]
X_phoneme_2=[]
for index in phenome_1:
    X_phoneme_1.append(X_full[index,0])
    X_phoneme_2.append(X_full[index,1])
X_phoneme_1=np.array(X_phoneme_1)
X_phoneme_2=np.array(X_phoneme_2)
finalshape=X_phoneme_1.shape
X_phoneme_1=np.reshape(X_phoneme_1,(finalshape[1]))
X_phoneme_2=np.reshape(X_phoneme_2,(finalshape[1]))
X_phoneme=np.zeros((len(X_phoneme_1),3))
X_phoneme[:,0]=X_phoneme_1
X_phoneme[:,1]=X_phoneme_2
X_phoneme[:,2]=X_phoneme_2+X_phoneme_1
...
for t in range(n_iter):
    ...
    for i in range(k):
        ...
        offst=0.5
        identity=np.identity(s.shape[0])
        offset=identity*offst
        s[i,:,:] = s[i,:,:]*offset
